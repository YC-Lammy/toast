...
use crate::ast_to_hir::TypeCheck;
use crate::common::{ClassId, FunctionId, VariableId};
use crate::error::Error;
use crate::hir::{
    ClassPropertyDesc, ClassType, Expr, FuncType, FunctionParam, PropNameOrExpr, Stmt, Type,
};
        ...
        for i in &class.implements {
            ...
            match impl_ty {
                Type::Interface(iface) => {
                    ...
                    self.type_checks.push(TypeCheck {
                        span: i.span,
                        ty: Type::Object(id),
                            ...
                            swc::ParamOrTsParamProp::Param(p) => {
                                ...
                                if let Some(ident) = p.pat.as_ident() {
                                    ...
                                    if let Some(ann) = &ident.type_ann {
                                        ...
                                        self.context
                                            .bind_variable(&ident.sym, param_id, ty, true, true);
                                    } else {
                                        // missing type annotation
                                        return Err(Error::syntax_error(
                                            ident.span,
                                ...
                                match &p.param {
                                    ...
                                    swc::TsParamPropParam::Ident(ident) => {
                                        ...
                                        if let Some(ann) = &ident.type_ann {
                                            ...
                                            // bind param to variable
                                            self.context.bind_variable(
                                                &ident.sym, param_id, ty, true, true,
                                            );
                                        } else {
                                            // missing type annotation
                                            return Err(Error::syntax_error(
