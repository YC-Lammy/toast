...
use crate::untyped_hir::{
    ...
    Callee, DeepClone, Expr, Function, Type,
};
...
impl GenericFunctionResolver {
    pub fn resolve_function_with_args(
        ...
        func: &mut Function,
        type_args: &[Type],
    ) -> Result<Option<Function>, Error<Span>> {
        ...
        for (i, g) in func.ty.generics.iter().enumerate() {
            let ty = if let Some(arg) = type_args.get(i) {
...
impl Visitor for GenericFunctionResolver {
    ...
    fn visit_expr(
        &mut self,
        expr: &mut crate::untyped_hir::Expr,
    ) -> Result<crate::untyped_hir::visit::BreakOrContinue, Self::Error> {
        match expr {
            ...
            Expr::Call {
                ...
                if let Some(method) = class.static_functions.iter_mut().find(|m| m.name.eq(prop)) {
                    let resolved =
                        self.resolve_function_with_args(*span, &mut method.function, type_args)?;
                    ...
                    if !type_args.is_empty() {
                        return Err(Error::syntax_error(
                            *span,
                            "dynamic call cannot have type arguments",
                        ));
                    };
                }
            }
