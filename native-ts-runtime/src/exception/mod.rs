...
/// LAM\0NATS
pub const NATIVE_TS_EXCEPTION_CLASS: u64 = u64::from_ne_bytes(['L' as u8, 'A' as u8, 'M' as u8, '\0' as u8, 'N' as u8, 'A' as u8, 'T' as u8, 'S' as u8]);
...
pub extern "C" fn __native_ts_eh_personality(
    ...
    let mut lsda = EndianSlice::new(unsafe { get_unlimited_slice(lsda as *const u8) }, NativeEndian);
    ...
    match eh_action{
        ...
        EHAction::Catch(landingpad) |
        EHAction::Cleanup(landingpad) => {
            ...
            #[cfg(not(any(target_arch="x86_64", target_arch="x86", target_arch="riscv64", target_arch="riscv32", target_arch="aarch64")))]
...
pub extern "C" fn __native_ts_allocate_exception(value: Any) -> *mut NativeTsException{
    unsafe{
        let exception = libc::malloc(core::mem::size_of::<NativeTsException>()) as *mut NativeTsException;
        ptr::write(
            exception, 
            NativeTsException{
...
pub extern "C" fn __native_ts_unwind_resume(exception: &mut NativeTsException){
    unsafe{
        unwinding::abi::_Unwind_Resume(&mut exception.header)
    }
}