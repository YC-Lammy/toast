...
use crate::ir_builder::{IRPackage, ir::IR};
...
pub fn remove_unread_variables(pkg: &mut IRPackage) {
    ...
    for (_id, func) in &mut pkg.functions{
        for removed in &removed_vars{
            if func.captures.contains(removed){
                func.captures = func.captures.iter().map(|v|*v).filter(|v| v.ne(removed)).collect();
            }
            if func.own_variables.contains(removed){
                func.own_variables = func.own_variables.iter().map(|v|*v).filter(|v| v.ne(removed)).collect();
            }
            if func.heap_variables.contains(removed){
                func.heap_variables = func.heap_variables.iter().map(|v|*v).filter(|v| v.ne(removed)).collect();
            }  
        }
    }
    ...
    for removed in &removed_vars{
        if pkg.global_variables.contains(removed){
            pkg.global_variables = pkg.global_variables.iter().map(|v|*v).filter(|v| v.ne(removed)).collect();
        }
        if pkg.heap_variables.contains(removed){
            pkg.heap_variables = pkg.heap_variables.iter().map(|v|*v).filter(|v| v.ne(removed)).collect();
        }
    }
}
...
impl IR{
    pub fn reads_acc(&self) -> bool{
        match self{
            ...
            Self::Call { arg_len:_, args:_, maybe_static:_ } => true,
            Self::CallStatic { func_id:_, arg_len:_, args:_ } => false,
            Self::CallStaticVarArgs { func_id:_, var_arg:_ } => false,
            Self::CallVarArgs { var_arg:_ } => true,
            Self::Continue { label:_ } => false,
            Self::CreateArgList(_) => false,
            Self::CreateArray { size:_ } => false,
            Self::CreateAsyncIter(_) => true,
            Self::CreateForInIter(_) => true,
            Self::CreateForOfIter(_) => true,
            Self::CreateObject => false,
            Self::CreateRegex { pattern:_, flag:_ } => false,
            Self::CreateVarArgList(_) => false,
            Self::Debugger => false,
            ...
            Self::LoadString(_) |
            Self::LoadTemp(_) |
            Self::LoadTpl { quasis:_, args_len:_, args:_ } |
            ...
            Self::Mod(_) => true,
            Self::Mul(_) => true,
            Self::New { arg_len:_, args:_ } => true,
            ...
            Self::Nullish(_) => true,
            Self::ObjAssign => true,
            Self::ObjCall { args:_, arg_len:_ } => true,
            Self::ObjCallVarArg { var_arg:_ } => true,
            Self::Or(_) => true,
            Self::Plus => true,
            Self::PushArg(_) => true,
            Self::PushVarArg(_) => true,
            Self::RShift(_) => true,
            ...
            Self::TryCatch => false,
            Self::TypeOf => true,
            Self::WriteComputed { obj:_, propname:_ } => true,
            Self::WriteField { object:_, key:_ } => true,
            Self::WriteIndex { object:_, index:_ } => true,
    ...
    pub fn writes_acc(&self) -> bool{
        match self{
            ...
            Self::BreakIfIterDone(_) => false,
            Self::Call { arg_len:_, args:_, maybe_static:_ } => true,
            Self::CallStatic { func_id:_, arg_len:_, args:_ } => true,
            Self::CallStaticVarArgs { func_id:_, var_arg:_ } => true,
            Self::CallVarArgs { var_arg:_ } => true,
            Self::Continue { label:_ } => false,
            Self::CreateArgList(_) => false,
            Self::CreateArray { size:_ } => true,
            Self::CreateAsyncIter(_) => false,
            Self::CreateForInIter(_) => false,
            Self::CreateForOfIter(_) => false,
            Self::CreateObject => true,
            Self::CreateRegex { pattern:_, flag:_ } => true,
            Self::CreateVarArgList(_) => false,
            Self::Debugger => false,
            Self::DeclareVar(_) => false,
            Self::Delete => true,
            ...
            Self::TryCatch => false,
            Self::TypeOf => true,
            Self::WriteComputed { obj:_, propname:_ } => false,
            Self::WriteField { object:_, key:_ } => false,
            Self::WriteIndex { object:_, index:_ } => false,
    ...
    pub fn is_call(&self) -> bool{
        match self{
            IR::Call { arg_len:_, args:_, maybe_static:_ } |
            IR::CallStatic { func_id:_, arg_len:_, args:_ } |
            IR::CallStaticVarArgs { func_id:_, var_arg:_ } |
            IR::CallVarArgs { var_arg:_ } |
            IR::ObjCall { args:_, arg_len:_ } |
            IR::ObjCallVarArg { var_arg:_ } => true,
            IR::New { arg_len:_, args:_ } => true,
            _ => false
        }
    }
